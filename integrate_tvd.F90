module integrator
  ...
  use sizes, only: neq,neuler,nrOfDim,mbc,RHO
  ...
  subroutine xintegr(itoterror)
    ...
    kloop: do k=sz,ez
       jloop :do j=sy,ey  ! integrate over all rows 
          ...
          do ieq=1,neq
             do i=1-mbc,mesh+mbc
                state1d(i,ieq)=stold(i+ioff,j,k,ieq)
             enddo
          enddo
          do i=1-mbc,mesh+mbc
             wp(i)=pressr(i+ioff,j,k)
          enddo
          ! advected quantities need to be densities
          do ieq=neuler+1,neq
             do i=1-mbc,mesh+mbc
                state1d(i,ieq)=state1d(i,ieq)*state1d(i,RHO)
             enddo
          enddo
          ...
          ! Report problems
          if (ierror /= 0) then
             write(30,*) 'TVD solver error (x), j,k= ',j,k
             call flush(30)
  ...
  subroutine yintegr(itoterror)
    ...
    kloop: do k=sz,ez
       iloop: do i=sx,ex      ! integrate over all rows 
          ...
          do ieq=1,neq
             do j=1-mbc,mesh+mbc
                state1d(j,ieq)=stold(i,j+joff,k,ieq)
             enddo
          enddo
          do j=1-mbc,mesh+mbc
          ...
          do ieq=neuler+1,neq
             do j=1-mbc,mesh+mbc
                state1d(j,ieq)=state1d(j,ieq)*state1d(j,RHO)
             enddo
          enddo
          ...
          ! Report problems
          if (ierror /= 0) then
             write(30,*) 'TVD solver error (y), i,k= ',i,k
             call flush(30)
             itoterror=itoterror+ierror
          endif
  ...
  subroutine zintegr(itoterror)
    ...
    jloop: do j=sy,ey
       iloop: do i=sx,ex      ! integrate over all rows 
          ...
          do ieq=1,neq
             do k=1-mbc,mesh+mbc
                state1d(k,ieq)=stold(i,j,k+koff,ieq)
             enddo
          enddo
          do k=1-mbc,mesh+mbc
          ...
          do ieq=neuler+1,neq
             do k=1-mbc,mesh+mbc
                state1d(k,ieq)=state1d(k,ieq)*state1d(k,RHO)
             enddo
          enddo
          ...
          ! Report problems
          if (ierror /= 0) then
             write(30,*) 'TVD solver error (z), i,j= ',i,j
             call flush(30)
             itoterror=itoterror+ierror
          endif
